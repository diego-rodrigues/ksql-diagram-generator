...
from ksql_item import KSQLItem, KSQLTable, KSQLStream
...
# ========================================================================
def draw_diagram(input_file:str, diagram_name:str, output_name:str):

  f = open(input_file, "r")
  inputs = f.read()

  p = KSQLParser()
  p.parseStatements(inputs)
  # p.print()
  ...
  with Diagram(diagram_name, show=True, filename=output_name, direction="LR", \
    ...
    for item in orderItems:
      ...
      if isinstance(item, KSQLTable):
        ...
        if (item.key == ""):
          draw_objects[item.name] = Node(label=p.multilines(item.name), height="0.6", \
                                      fixedsize="false", labelloc="c", shape="rectangle", \
                                      color="black", style="filled", fillcolor="peachpuff")
        else:
          draw_objects[item.name] = Node(label=p.multilines(item.name), xlabel=p.multilines(item.key,30), height="0.6", \
                                      fixedsize="false", labelloc="c", shape="rectangle", \
                                      color="black", style="filled", fillcolor="peachpuff")
        
      # streams
      elif isinstance(item, KSQLStream):
        draw_objects[item.name] = Node(label=p.multilines(item.name), xlabel=p.multilines(item.key), height="0.6", \
